...
| Qwen2.5-3B base | teacher (1.2k tok) | 100% | 55.3% | 55.3% | – |
| **CabinCopilot-3B (ours)** | student (60 tok) | – | – | – | – |

Baselines (n=150 held-out scenarios, local Ollama Q4_K_M on a 2019 Intel MacBook):
with the short student prompt the base model can barely hold the contract
(p50 5.6 s); the full engineered prompt buys schema validity but only 55% safety
compliance, and raises p50 latency 54% to 8.6 s — prompting buys format, not
judgment, and pays prompt tokens on every call. The fine-tune has to deliver
teacher-prompt behaviour (or better) at student-prompt latency.
